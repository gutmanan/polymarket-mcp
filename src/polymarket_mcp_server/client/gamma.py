...
class GammaClient:
    ...
    def parse_pydantic_market(self, market_object: dict) -> Market | None:
        try:
    ...
    def parse_nested_event(self, event_object: dict()) -> PolymarketEvent | None:
    ...
    def parse_pydantic_event(self, event_object: dict) -> PolymarketEvent | None:
        try:
            if "tags" in event_object:
                print("tags here", event_object["tags"])
                tags: list[Tag] = []
    ...
    def get_events(self, querystring_params={}, parse_pydantic=False, local_file_path=None) -> list[PolymarketEvent] | None | Any:
        if parse_pydantic and local_file_path is not None:
            raise Exception(
                'Cannot use "parse_pydantic" and "local_file" params simultaneously.'
            )
